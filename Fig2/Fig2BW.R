...
comp.dat <- read.csv(paste0(homewd,"/fruit-bat-morphology-literature.csv"), header=T, stringsAsFactors = F)
head(comp.dat)
...
dat1 <- read.csv(paste0(homewd, "/morph_paper_dat_7_29_2021.csv"), header=T, stringsAsFactors = F)
head(dat1)
...
colz[names(colz)=="Pteropus rufus"] = "gray50"
colz[names(colz)=="Eidolon dupreanum"] = "grey0"
colz[names(colz)=="Rousettus madagascariensis"] = "grey45"
...
female.mod.mada <- lm(log10(bat_weight_g) ~ log10(bat_forearm_mm), data=subset(all.dat, bat_sex=="F" & origin=="Malagasy Pteropodids"))
male.mod.mada <- lm(log10(bat_weight_g) ~ log10(bat_forearm_mm), data=subset(all.dat, bat_sex=="M" & origin=="Malagasy Pteropodids"))


female.mod.nonmada <- lm(log10(bat_weight_g) ~ log10(bat_forearm_mm), data=subset(all.dat, bat_sex=="F" & origin=="All Pteropodids"))
...
all.dat$predicted_mass[all.dat$bat_sex=="F" & all.dat$origin=="Malagasy Pteropodids" & !is.na(all.dat$bat_weight_g) & !is.na(all.dat$bat_forearm_mm)] <- 10^predict(female.mod.mada)
all.dat$predicted_mass[all.dat$bat_sex=="M" & all.dat$origin=="Malagasy Pteropodids" & !is.na(all.dat$bat_weight_g) & !is.na(all.dat$bat_forearm_mm)] <- 10^predict(male.mod.mada)
all.dat$predicted_mass[all.dat$bat_sex=="F" & all.dat$origin=="All Pteropodids" & !is.na(all.dat$bat_weight_g) & !is.na(all.dat$bat_forearm_mm)] <- 10^predict(female.mod.nonmada)
all.dat$predicted_mass[all.dat$bat_sex=="M" & all.dat$origin=="All Pteropodids" & !is.na(all.dat$bat_weight_g) & !is.na(all.dat$bat_forearm_mm)] <- 10^predict(male.mod.nonmada)


#do CIs later
#and predict across your dataset
all.dat$predicted_mass_lci <- NA
all.dat$predicted_mass_lci[all.dat$bat_sex=="F" & all.dat$origin=="Malagasy Pteropodids" & !is.na(all.dat$bat_weight_g) & !is.na(all.dat$bat_forearm_mm)] <- 10^(predict(female.mod.mada) - (1.96*predict(female.mod.mada, type="response", se.fit = T)$se))
all.dat$predicted_mass_lci[all.dat$bat_sex=="M" & all.dat$origin=="Malagasy Pteropodids" & !is.na(all.dat$bat_weight_g) & !is.na(all.dat$bat_forearm_mm)] <- 10^(predict(male.mod.mada) - (1.96*predict(male.mod.mada, type="response", se.fit = T)$se))
all.dat$predicted_mass_lci[all.dat$bat_sex=="F" & all.dat$origin=="All Pteropodids" & !is.na(all.dat$bat_weight_g) & !is.na(all.dat$bat_forearm_mm)] <- 10^(predict(female.mod.nonmada) - (1.96*predict(female.mod.nonmada, type="response", se.fit = T)$se))
all.dat$predicted_mass_lci[all.dat$bat_sex=="M" & all.dat$origin=="All Pteropodids" & !is.na(all.dat$bat_weight_g) & !is.na(all.dat$bat_forearm_mm)] <- 10^(predict(male.mod.nonmada) - (1.96*predict(male.mod.nonmada, type="response", se.fit = T)$se))
all.dat$predicted_mass_lci[all.dat$predicted_mass_lci<0] <- 0

all.dat$predicted_mass_uci <- NA
all.dat$predicted_mass_uci[all.dat$bat_sex=="F" & all.dat$origin=="Malagasy Pteropodids" & !is.na(all.dat$bat_weight_g) & !is.na(all.dat$bat_forearm_mm)] <- 10^(predict(female.mod.mada) + (1.96*predict(female.mod.mada, type="response", se.fit = T)$se))
all.dat$predicted_mass_uci[all.dat$bat_sex=="M" & all.dat$origin=="Malagasy Pteropodids" & !is.na(all.dat$bat_weight_g) & !is.na(all.dat$bat_forearm_mm)] <- 10^(predict(male.mod.mada) + (1.96*predict(male.mod.mada, type="response", se.fit = T)$se))
all.dat$predicted_mass_uci[all.dat$bat_sex=="F" & all.dat$origin=="All Pteropodids" & !is.na(all.dat$bat_weight_g) & !is.na(all.dat$bat_forearm_mm)] <- 10^(predict(female.mod.nonmada) + (1.96*predict(female.mod.nonmada, type="response", se.fit = T)$se))
all.dat$predicted_mass_uci[all.dat$bat_sex=="M" & all.dat$origin=="All Pteropodids" & !is.na(all.dat$bat_weight_g) & !is.na(all.dat$bat_forearm_mm)] <- 10^(predict(male.mod.nonmada) + (1.96*predict(male.mod.nonmada, type="response", se.fit = T)$se))
...
pall_2 <- ggplot(data=all.dat) + 
  coord_cartesian(ylim = c(0,1200), xlim=c(0,200)) +
  scale_color_manual(values=colz) +
...
ggsave(file = paste0(homewd, "/final-figures/eps_tiff/Fig2BW.eps"),
       units="mm",  
       width=90, 
       height=60, 
       scale=3, 
...
t_EarAllBAtsMvF <- cbind.data.frame(meanM = out$estimate[2], meanF = out$estimate[1], p_val_t_test = out$p.value) 
...
t_EarPteropus <- cbind.data.frame(meanM = out$estimate[2], meanF = out$estimate[1], p_val_t_test = out$p.value) 
...
t_EarEid <- cbind.data.frame(meanM = out$estimate[2], meanF = out$estimate[1], p_val_t_test = out$p.value) 
rownames(t_EarEid) <- c()
...
t_EarRou <- cbind.data.frame(meanM = out$estimate[2], meanF = out$estimate[1], p_val_t_test = out$p.value) 
rownames(t_EarRou) <- c()
...
t_TibAllBAtsMvF <- cbind.data.frame(meanM = out$estimate[2], meanF = out$estimate[1], p_val_t_test = out$p.value) 
rownames(t_EarAllBAtsMvF) <- c()
...
t_TibPteropus <- cbind.data.frame(meanM = out$estimate[2], meanF = out$estimate[1], p_val_t_test = out$p.value) 
...
t_TibEid <- cbind.data.frame(meanM = out$estimate[2], meanF = out$estimate[1], p_val_t_test = out$p.value) 
rownames(t_TibEid) <- c()
...
t_TibRou <- cbind.data.frame(meanM = out$estimate[2], meanF = out$estimate[1], p_val_t_test = out$p.value) 
rownames(t_TibRou) <- c()
...
t_ForAllBAtsMvF <- cbind.data.frame(meanM = out$estimate[2], meanF = out$estimate[1], p_val_t_test = out$p.value) 
...
t_ForPteropus <- cbind.data.frame(meanM = out$estimate[2], meanF = out$estimate[1], p_val_t_test = out$p.value) 
...
t_ForEid <- cbind.data.frame(meanM = out$estimate[2], meanF = out$estimate[1], p_val_t_test = out$p.value) 
rownames(t_ForEid) <- c()
...
t_ForRou <- cbind.data.frame(meanM = out$estimate[2], meanF = out$estimate[1], p_val_t_test = out$p.value) 
rownames(t_ForRou) <- c()
