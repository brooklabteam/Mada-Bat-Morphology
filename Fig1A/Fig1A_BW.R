...
p1<-ggplot() +  
  geom_sf(color = "gray90", fill = "gray90",data = orotl_shp)+
...
loko<-c("Rousettus madagascariensis"="black","Eidolon dupreanum"="white","Pteropus rufus"="gray")
...
p4 <- p2b+geom_path(data = pies, mapping = aes(x = x, y = y, group = roost_site), size = 0.25) + #tsy misy haiko aloha ny tena ilaina azy
  ...
  geom_scatterpie(aes(x=x., y=y., group = roost_site, r = log10(nombre/20)), data = pies,
                  cols=c("bat_species"), long_format=TRUE, color = "black")+ 
...
ggsave(file = paste0(homewd, "/final-figures/Fig1A_final_mapBW.pdf"),
       plot=p4,
       units="mm",  
       width=60, 
       height=50, 
