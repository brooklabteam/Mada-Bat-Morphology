...
dat <- read.csv(paste0(homewd, "/morph_paper_dat_7_29_2021.csv"), header=T, stringsAsFactors = F)
head(dat)
...
ColM<- c("Pteropus rufus"="gray50", "Eidolon dupreanum"="grey45","Rousettus madagascariensis"="grey0")
...
ColM<- c("Pteropus rufus"="gray50", "Eidolon dupreanum"="grey0","Rousettus madagascariensis"="grey45")
...
p4_main <- ggplot(data = new.All.dat) + 
  geom_ribbon(data = seas.dat, aes(x=x, ymin=-Inf, ymax=Inf),fill="gray75", alpha=0.3)+
  geom_ribbon(data = preg.dat, aes(x=x, ymin=-Inf, ymax=Inf),fill="gray60", alpha=0.3)+
...
ggsave(file = paste0(homewd, "/final-figures/eps_tiff/Fig3BW.tiff"),
       plot = p4_main,
       units="mm",  
       width=80, 
       height=60, 
