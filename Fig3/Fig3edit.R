...
#but save as standarized mass
median.mass =ddply(AllsppAdults1, .(bat_species, bat_sex), summarise, median_mass=median(bat_weight_g, na.rm=T))

#add unique median mass for species and sex
AllsppAdults1 <- merge(AllsppAdults1, median.mass, by=c("bat_species", "bat_sex"), all.x = T, sort = F)
head(AllsppAdults1)
AllsppAdults1$standard_mass_g <- AllsppAdults1$bat_weight_g/AllsppAdults1$median_mass
AllsppAdults1$standard_mass_g

ggplot(data=AllsppAdults1) + geom_point(aes(x=standard_mass_g, y=bat_forearm_mm, color=bat_sex))
...
modAllFmixed <- lmerTest::lmer(log10(bat_weight_g)~log10(bat_forearm_mm) +(1|bat_species), data=subset(AllsppAdults1, bat_sex=="female"))
...
AllsppAdults1$prediction[AllsppAdults1$bat_sex=="female"] <- 10^predict(modAllFmixed)
AllsppAdults1$prediction[AllsppAdults1$bat_sex=="male"] <- 10^predict(modAllMmixed)
